...
class CheckinService {
  ...
  Future<CheckinModel> performCheckin({
    ...
    // Verifica se o desafio ainda está ativo (se for um check-in de grupo)
    ...
    // Verifica se já fez check-in hoje NESTE GRUPO ou GERALMENTE
    ...
    if (groupId != null && groupId.isNotEmpty) {
      ...
      // Se for um check-in pessoal, a gente verifica se ele ja tem algum check-in pessoal hoje
      query = query.where('groupId', isNull: true);
    }

    final existingCheckin = await query.limit(1).get();
  ...
  Future<bool> hasCheckedInToday(String userId, {String? groupId}) async {
    final today = _dateFormat.format(DateTime.now());
  ...
  Future<List<CheckinModel>> getRecentCheckins({
    ...
    Query<Map<String, dynamic>> query = _firestore.collection('checkins');
  ...
  Future<int> getUserScore(String userId, {String? groupId}) async {
    Query<Map<String, dynamic>> query = _firestore
        .collection('checkins')
        .where('userId', isEqualTo: userId);
